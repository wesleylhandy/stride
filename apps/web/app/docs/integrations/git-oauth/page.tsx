...
export const metadata: Metadata = {
  title: 'Git OAuth Integration - Stride',
  description: 'Configure GitHub or GitLab OAuth for repository integration',
};

async function getDocContent(): Promise<ParsedDoc> {
  // Path from apps/web to repo root
  const repoRoot = join(process.cwd(), '..', '..');
  const filePath = join(repoRoot, 'docs', 'integrations', 'git-oauth.md');
...
export default async function GitOAuthDocsPage() {
  ...
  return (
    <PageContainer variant="constrained">
      <DynamicDocumentationPageContent
        title="Git OAuth Integration"
        description="Configure GitHub or GitLab OAuth for repository webhook integration and automatic issue status updates"
        sections={sections}
        activeSection="git-oauth"
        content={content}
        enableMermaid={false}
