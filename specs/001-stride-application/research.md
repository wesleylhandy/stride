...
#### Prompt Strategy
- **Structure**: Single system prompt + structured user message
- **System Prompt**: Stored in markdown file (e.g., `packages/ai-gateway/prompts/system-prompt.md`) within repository, version controlled. AI Gateway service reads prompt file at startup. Defines AI role (issue triage specialist), output format requirements (JSON with summary, priority, suggestedAssignee), analysis guidelines (focus on root cause, consider error traces, match project priority values). Includes JSON schema definition and one example output to improve consistency and reduce parsing errors. Handles all triage scenarios (error analysis, general issues, different issue types) - no separate prompts needed. Markdown format enables easy editing and external testing (Google AI Studio, OpenAI dashboard) before updating. Changes require service restart to take effect.
- **Output Format Specification**: JSON schema + example included in system prompt. Schema defines required structure (summary: string, priority: string, suggestedAssignee: string). Example demonstrates expected format and quality. If provider supports structured output modes (e.g., OpenAI JSON mode, Anthropic structured outputs), use them when available for better reliability. Keep schema/examples in prompt for compatibility across all providers (including Ollama which may not support structured modes).
- **User Message**: Structured issue context payload (title, description, status, custom fields, error traces if available, recent comments - last 5-10). Project-specific context (priority values, workflow rules, custom field definitions) injected into this payload to customize AI behavior without modifying system prompt. Context adapts to scenario (error traces included when available, issue type from custom fields).
- **Implementation Approach**: Direct LLM API calls (no agent frameworks or tool definitions). Use native provider APIs (OpenAI, Anthropic, Ollama) directly with system + user message structure. Leverage provider structured output modes when available (OpenAI JSON mode, Anthropic structured outputs) for better reliability. Avoids complexity of agent orchestration, tool definitions, and retry logic that agent frameworks introduce. Sufficient for single-purpose triage task with structured output.
- **Rationale**: Single system prompt stored in markdown file is simpler to maintain and update than per-project prompt templates or scenario-specific prompts. Markdown format enables easy editing, version control, and external testing workflows. JSON schema + example in prompt improves output consistency and reduces parsing errors. Project customization achieved through data injection in user message, not prompt modification. Single prompt handles all scenarios by including relevant context in user message. Direct API calls avoid unnecessary abstraction layers. Aligns with KISS principle - sufficient for structured output without over-engineering.
