...
### Session 2026-01-23 (Phase 9 AI Prompt Strategy Clarifications)

- Q: How should prompts be structured for the AI triage implementation? → A: Single system prompt + structured user message. Use one system prompt that defines the AI's role, output format requirements, and analysis guidelines. Issue context (title, description, status, custom fields, error traces, recent comments) is sent as a structured user message. This approach balances simplicity with effectiveness - single system prompt is easier to maintain and update, while structured user message ensures consistent context delivery. Avoids complexity of multiple specialized prompts or agent frameworks for this single-purpose triage task.
- Q: Should the AI implementation use agent frameworks or tool definitions? → A: No agent/tool framework - direct LLM API calls. Use direct LLM API calls with system + user prompts, avoiding agent frameworks (e.g., LangChain agents) or tool definitions. For a single-purpose triage task with structured output, direct calls are sufficient and simpler. Agent frameworks add unnecessary complexity (tool definitions, orchestration, retry logic) without clear benefit for this use case. Aligns with KISS principle - keep implementation simple and maintainable.
- Q: Should the system prompt be configurable per-project or hardcoded? → A: Single system prompt stored in markdown file with project context injection. System prompt is stored in a markdown file (e.g., `packages/ai-gateway/prompts/system-prompt.md`) within the repository for consistency and easier maintenance. Project-specific context (priority values, workflow rules, custom field definitions) is injected into the structured user message payload. This approach keeps prompts consistent across projects while allowing project customization through data injection, not prompt modification. Avoids complexity of storing/managing prompt templates per project while still supporting project-specific needs. Prompt file is version controlled and can be easily tested externally (Google AI Studio, OpenAI dashboard) before updating.
- Q: Should there be separate prompts for different scenarios (error analysis vs general triage)? → A: Single prompt handles all scenarios. One system prompt covers all triage scenarios (issues with errors, general issues, different issue types). The structured user message already includes relevant context (error traces when available, issue type, custom fields), allowing the AI to adapt its analysis appropriately. Separate prompts would add maintenance overhead without clear benefit. Single prompt is simpler and sufficient for the triage use case.
- Q: How should output format be specified in the prompt (schema, examples, or instructions only)? → A: JSON schema + example in system prompt. Include JSON schema definition and one example output in the system prompt. Schema defines required structure (summary, priority, suggestedAssignee fields), example demonstrates expected format and quality. This approach improves consistency and reduces parsing errors. If provider supports structured output modes (e.g., OpenAI JSON mode), use them when available, but keep schema/examples in prompt for compatibility across all providers.
- Q: Where should the system prompt be stored for easy maintenance and external testing? → A: Markdown file in repository. System prompt stored in markdown file (e.g., `packages/ai-gateway/prompts/system-prompt.md`) within the repository, version controlled alongside code. This enables easy editing, external testing in AI Studio/dashboards (copy-paste workflow), and maintains configuration-as-code principles. Prompt file is read by AI Gateway service at startup.
- Q: How should prompt file updates be applied to the running service? → A: Requires service restart. AI Gateway service reads the markdown prompt file at startup. Changes to the prompt file require service restart/redeployment to take effect. This approach is simplest, safest, and aligns with containerized deployment patterns. Prompt updates are infrequent enough that restart overhead is acceptable.
