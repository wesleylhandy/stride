...
1. **Given** an issue exists with sufficient context (description, error traces, comments), **When** a user clicks "Triage with AI" or "Run AI Triage", **Then** the system sends the issue context to the configured AI gateway
...
3. **Given** AI analysis is requested, **When** the gateway returns results, **Then** the issue view displays a plain-language summary of the likely root cause
4. **Given** AI analysis completes, **When** results are returned, **Then** the system suggests priority level and potential assignee based on issue content and team member expertise
5. **Given** AI suggestions are provided, **When** viewing the issue, **Then** users can accept, modify, or dismiss the AI recommendations
...
- What happens when the AI gateway returns malformed or incomplete responses?
...
- **FR-028**: System MUST support configuring an AI gateway endpoint for triage and analysis
- **FR-029**: System MUST send issue context to the AI gateway when triage is requested
- **FR-030**: System MUST display AI-generated summaries, priority suggestions, and assignment recommendations
...
- **TC-001**: System MUST use a single PostgreSQL database with connection pooling for all data persistence. All entities (issues, users, sprints, configuration, repository connections) are stored relationally in PostgreSQL.
